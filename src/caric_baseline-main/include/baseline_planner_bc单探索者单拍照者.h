...
public:
    ...
    */
    ...
    void Astar_photo_my(Eigen::Vector3d target, string myname, bool &flag)
    {
    ...
    Eigen::Vector3d get_next_point_mn(bool& astar_finish, bool& flag)//得到下一路径点的全局坐标
    {
        ...
        if(std::sqrt((now_position_global[0]-target_point_mn_last[0])*(now_position_global[0]-target_point_mn_last[0]) \
                    + (now_position_global[1]-target_point_mn_last[1])*(now_position_global[1]-target_point_mn_last[1])\
                    + (now_position_global[2]-target_point_mn_last[2])*(now_position_global[2]-target_point_mn_last[2])) < 1) {
            flag = true;
        }
        ...
        if (!path_global.empty())  // 路径点不为空
        {
            ...
            else
            {
                if (!box_viewpoint_list.empty()) {
                    ...
                    last_viewpoint = path_global.front();
                }
    ...
    Eigen::Vector3d get_next_point(bool global, Eigen::Vector3d now_global_position, bool flag = false, string myname = "name")  //得到下一路径点的全局坐标
    {
        ...
        if (!box_viewpoint_list.empty())  // 路径点不为空
        {
            if (!get_first_viewpoint) {
                ...
                std::cout << "得到第一个视点" << std::endl;
            }
            else if(std::sqrt((now_position_global[0]-last_viewpoint[0])*(now_position_global[0]-last_viewpoint[0]) \
                    + (now_position_global[1]-last_viewpoint[1])*(now_position_global[1]-last_viewpoint[1])\
                    + (now_position_global[2]-last_viewpoint[2])*(now_position_global[2]-last_viewpoint[2])) < 1 || flag) {
                last_viewpoint = box_viewpoint_list.front();
                box_viewpoint_list.pop_front();
                std::cout << "=====================>>> 得到下一个视点: " << count_view_i << "还剩下视点数目：" << box_viewpoint_list.size() << std::endl;
                count_view_i++;
                ...
                std::cout << "当前位置为：" << now_position_global[0] << ", " << now_position_global[1] << ", "<< now_position_global[2] << ", "<< std::endl;
                std::cout << "目标视点位置为：" << last_viewpoint[0] << ", " << last_viewpoint[1] << ", "<< last_viewpoint[2] << ", "<< std::endl;
                std::cout << "当前位置距离目标点为：" << dis << std::endl;
            }
            // double dis = std::sqrt((now_position_global[0]-last_viewpoint[0])*(now_position_global[0]-last_viewpoint[0]) + (now_position_global[1]-last_viewpoint[1])*(now_position_global[1]-last_viewpoint[1]) + (now_position_global[2]-last_viewpoint[2])*(now_position_global[2]-last_viewpoint[2]));
            // std::cout << "当前位置为：" << now_position_global[0] << ", " << now_position_global[1] << ", "<< now_position_global[2] << ", "<< std::endl;
    ...
    void exploration_layer(string myname, int region_index, int high)  //探索无人机更新path_index
    {
        ...
        if (path_index_temp.empty())//当没有搜索到路径或者边界时
        {
            ...
            {
                // 探索完成后，需要和拍照无人机进行通信，告知可以去拍照了，所以在当前的这个层上飞到和飞入点同样的xy坐标上，
                if (local_dict[follower[region_index]].state != 1 && local_dict[follower[region_index]].state != 2)  // 如果这个区域的拍照无人机的状态不是1，就是完成了拍照或者处于等待状态
                {
    ...
    {
       ////yolo();
        // for (int i = 0; i < finish_flag.size(); i++)  // 前面将每个边界框按照无人机数量进行了分块n，这里对n-1块进行遍历，最后一块直接由探索无人机去拍照
        // {
        //     if (finish_flag[i] == 0)  // 一块一块探索
        //     {
        ...
        // explor_finish = true;
        if (finish_flag_i == 0) {
            exploration_layer(myname, 0, map_shape.z() / 2);
            return;
        }
    ...
    void update_viewpoint_box(vector<Eigen::Vector3d>& box_vp_tem) {  // 从视点节点中获取视点信息（探索者调用）
        ...
        viewpoint_save = true;
        std::cout << "update_viewpoint()成功：" << box_viewpoint_list.size() << std::endl;
    }

    void update_viewpoint_box_from_topic(vector<Eigen::Vector3d>& box_vp_tem) {  // 从探索者无人机中接受视点信息
...
public:
    ...
    void replan()//根据无人机当前状态与无人机类型，重新进行路径规划更新二次规划后的信息
    {
        ...
        // if (state != pre_state || is_transfer != pre_it) {
        //     std::cout << namespace_ << " state: " << state << std::endl;
        //     std::cout << namespace_ << " is_transfer: " << is_transfer << std::endl;
        //     std::cout << namespace_ << " now_id: " << now_id << std::endl;
        // }
        // pre_state = state;
        // pre_it = is_transfer;
        ...
        else
        {
            ...
            {
               ////yolo();
                state = map_set[now_id].get_state_leader();  // 返回领导者状态，含义？？
                map_set[now_id].exploration(namespace_);  // 探索区域
                if(map_set[now_id].update_finish_id()) {
                    finish_id = now_id;
                }

                // if(explor_finish && pcd_id == now_id && map_set.size() > now_id) {  // for pcd
                //     mtx_cloud.lock();
                //     pcl::PointCloud<pcl::PointXYZ>::Ptr pcd_cloud(new pcl::PointCloud<pcl::PointXYZ>);
                //     string pcd_name = "/home/ztd/ws_caric/src/caric_baseline-main/data/" + namespace_.substr(1, namespace_.length() - 1) + "_box_" + to_string(now_id) + ".pcd";
                //     for (auto it = pointSet.begin(); it != pointSet.end(); ++it) {
                //         const Point3D& point = *it;
                //         pcd_cloud->push_back(pcl::PointXYZ(point.x, point.y, point.z));
                //     }
                //     pcl::io::savePCDFile(pcd_name, *pcd_cloud);
                //     // 读取点云集中的数据并存成pcd文件
                //     explor_finish = false;
                //     std::cout << "save pcd " + namespace_ + "box: " << now_id << endl;
                //     pointSet.clear();
                //     pcd_id++;
                //     mtx_cloud.unlock();
                // }
               ...
               ////yolo();
                ...
                {
                    // if(pcd_id == now_id && map_set.size() > now_id) {  // for pcd
                    //     mtx_cloud.lock();
                    //     pcl::PointCloud<pcl::PointXYZ>::Ptr pcd_cloud(new pcl::PointCloud<pcl::PointXYZ>);
                    //     string pcd_name = "/home/ztd/ws_caric/src/caric_baseline-main/data/" + namespace_.substr(1, namespace_.length() - 1) + "_box_" + to_string(now_id) + ".pcd";
                    //     for (auto it = pointSet.begin(); it != pointSet.end(); ++it) {
            ...
            else
            {
                ...
                if (map_set.size() > now_id)
                {
                    // std::cout << "taking photo viewpoint, now_id: " << now_id << std::endl;
                    // map_set[now_id].take_photo(namespace_);  // 改写这里，接入视点
                    // map_set[now_id].take_photo_viewpoint(namespace_);
                    // path_show = map_set[now_id].get_path_show();
                    static bool flag = false;  // 用于判断何时更新点
                    Eigen::Vector3d target = map_set[now_id].get_next_point(false, now_global_position, flag, namespace_);  // 如果到达目标点, 得到下一个点
                    // get_way_point = update_target_waypoint(now_global_position);
                    
                    if (!Astar_finish || flag) {
                        global_map.Astar_photo_my(target, namespace_, flag);  // A*规划,下一个点不可达则flag为true
        ...
        if (odom_get && finish_init)
        {
            ...
            else  // 局部，正在拍照或者探索
            {
                if (is_leader) {
                    ...
                    target_position = map_set[now_id].get_next_point(false, now_global_position);
                }
                finish_first_planning = true;
                return true;
            }
    ...
    bool update_target_waypoint(Eigen::Vector3d now_global_position)//更新目标路径点
    {
        if (odom_get && finish_init)
        {
            if (is_transfer || map_set.size() == now_id)  // 在边界框直接跳转
        ...
        else if (topic == "flyin")
        {
            ...
            if (stoi(target_team) == Teamid) {
                ...
                for (int i = 0; i < stoi(box_num); i++) {
                    ...
                    map_set[i].update_viewpoint_box_from_topic(tem_vec);
                }
            }
